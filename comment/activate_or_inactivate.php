...
<head>
	<title>Section > Equipment > Change Comment Activate/Deactivate Status</title>
</head>
...
<br /> <h2> Section > Equipment > Change Comment Activate/Deactivate Status</h2> <hr />
...
<?php
	...
	{	
		...
		$sql_commentActiveFlag = "SELECT comment_activeFlag FROM comment WHERE comment_id=$comment_id"; 
		$results_commentActiveFlag = mysqli_query($link,$sql_commentActiveFlag);
		echo (!$results_commentActiveFlag?die(mysqli_error($link)."<br />$sql_commentActiveFlag"):"");	
		list($comment_activeFlag) = mysqli_fetch_array($results_commentActiveFlag);
		...
		echo "<br /><br />";
		echo "<b>Is currently set to: </b>"; if ($comment_activeFlag == 1) {echo "Active"; } else { echo "Removed"; }
		echo "<br /><br /><br />";
		
		if ($comment_activeFlag == 1) { $formButtonName = "Remove Comment"; }  else { $formButtonName = "Activate Comment"; } 
		if ($comment_activeFlag == 1) { $newFormValue = 0; }  else { $newFormValue = 1; }
		
		// Start Comment Form
		echo "<form method='post' action=''>";
		echo "<input type='hidden' name='comment_activeFlag' value='$newFormValue'>"; // Value of 0 = Set Comment to Hide | Value of 1 = Set Comment to Show
		...
		if (isset($_POST['comment_activeFlag']) and $_POST['comment_activeFlag']!="")
		{
			$comment_activeFlag = $_POST['comment_activeFlag']; 
		
			$sql_hideComment = "UPDATE comment SET comment_activeFlag=$comment_activeFlag WHERE comment_id=$comment_id";
			...
			$sql_checkComment = "SELECT comment_id FROM comment WHERE comment_id=$comment_id AND comment_value='$comment_value' AND comment_activeFlag=$comment_activeFlag";
			...
			if ($comment_activeFlag == 1) { $newStatusUpdate = "Active"; }  else { $newStatusUpdate = "Remove"; } 
