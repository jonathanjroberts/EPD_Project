...
<?php
	$section_id = intval($_GET['q']);

	$sql_sectionList = "SELECT section.section_id,section.section_name,section.area_id 
			FROM section,equipment 
			WHERE section.section_id=$section_id AND section.section_activeFlag=1 AND equipment.section_id=section.section_id";
			

	$results_sectionList = mysqli_query($link,$sql_sectionList);
			echo (!$results_sectionList?die(mysqli_error($link)."<br />$sql_sectionList"):"");
	
	while(list($section_id,$section_name,$areaId) = mysqli_fetch_array($results_sectionList)){
				$input_section[$section_id] = $section_name; 
				$area_id = $areaId; // not the best way to pust AreaId, but we know there's only 1 possible value
			}
