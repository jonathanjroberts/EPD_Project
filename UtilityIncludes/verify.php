<?php
		...
		// The following include will retreive $secret variable.
		...
		if (isset($_COOKIE['userId']) and isset($_COOKIE['userName']) and isset($_COOKIE['timeOfLogin']) and isset($_COOKIE['hash'])){
			...
			if ($hashCookie != $hashCalculated) {
				...
				exit;
			} elseif ( $role < 1 and strpos(strtolower($_SERVER["REQUEST_URI"]),"admin" ) ) {
				header('location:' . $base_url . '/login.php');
				exit;
			}
		} else {
		...
		if ($role == 1) 
		{ 
			$adminAccountOptionsStr = "||   <a href=$base_url/admin/default.php>Admin Menu</a>"; 
		} else {
			$adminAccountOptionsStr = "";
		}
		
		$userAccountOptionsStr = "<br /> Welcome: <b>$userName</b> - <a href=$base_url/logout.php>Log Out</a>   $adminAccountOptionsStr <br /><br />";
			
		echo $userAccountOptionsStr;
