...
<head>
	<title>Admin > Menu > Module > List</title>
	<style> 
...
<!-- http://localhost/PMBA/EPD/public/admin/module/list.php -->

<br /> <h2> Admin > Menu > Module > List</h2> <hr />
...
<?php
	...
	$sql="SELECT module_id,module_name,area_id,module_activeFlag FROM module";
	$results = mysqli_query($link,$sql);
	echo (!$results?die(mysqli_error($link)."<br />$sql"):"");
	
	echo "<table>";
	echo "<tr>";
	echo "<th width='150px'>Module Id</th>";
	...
	while(list($module_id,$module_name,$area_id,$module_activeFlag) = mysqli_fetch_array($results)){
		echo "<tr>";
		echo "<td>$module_id</td> <td>$area_id</td>";
		echo "<td>$module_name</td>";
		echo "<td>"; if ($module_activeFlag == 1) { echo "Active"; } else { echo "Inactive"; } echo "</td>";
		echo "<td>";
		echo "<a href='edit.php?module_id=$module_id'>Edit</a> | ";	
		if ($module_activeFlag == 1) {  
			echo "<a href='activate_or_inactivate.php?module_id=$module_id'>Inactivate</a>";
		} else {
			echo "<a href='activate_or_inactivate.php?module_id=$module_id'>Activate</a>";
		}
		echo "</td>";
		echo "</tr>";
	}
