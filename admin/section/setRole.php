...
<?php
	...
	{	
		$section_id = $_GET['user_id'];

		// Storing Passed Section Name  
		$sql = "SELECT user_id,user_name,area_id,user_activeFlag,role FROM users";
		$results = mysqli_query($link,$sql_section);
		echo (!$results?die(mysqli_error($link)."<br />$sql_section"):"");	
		...
		if ($rolw == 1) { $newFormValue = 0; }  else { $newFormValue = 1; }
